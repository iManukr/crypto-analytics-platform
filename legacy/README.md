> ## Archived — superseded by the platform in the repository root
>
> This is **Phase 1**: a single producer and consumer streaming Binance candles
> through Aiven-hosted Kafka into Aiven-hosted PostgreSQL, with a Grafana
> dashboard on top. It worked, but it depended on managed cloud services, ran as
> two scripts in two terminals, and had no transformation layer, no tests, no
> orchestration and no CDC.
>
> The project in the repository root supersedes it completely: the same public
> data source, but landed in a local PostgreSQL, replicated into ClickHouse by
> Debezium CDC, modelled with dbt, orchestrated by Airflow, and started with one
> `docker compose up`. See the root [README.md](../README.md).
>
> **These files are kept for history and are not maintained.** Two notes if you
> want to run them anyway:
>
> * `.env` now lives in this directory rather than the repository root, so run
>   the scripts from here (`cd legacy && python producer.py`) or `load_dotenv()`
>   will not find it. It was moved so that `docker compose` in the root cannot
>   pick up the Aiven credentials and inject them into the new stack.
> * They still require live Aiven Kafka and PostgreSQL services plus the TLS
>   certificates in `../certificates/`. Neither is needed by the new platform.
...
- **Producer** fetches the most recent *closed* 1-minute candle from Binance every
  ...
  It also **heals gaps**: on startup it republishes the last `BACKFILL_MINUTES`
  of candles, and while running it detects gaps (sleep, network drop) and
  backfills them before continuing. Re-sending is safe — the consumer upserts.
...
# How many minutes the producer republishes on startup (0 disables)
...
Every outage leaves a hole: Binance is polled live, so minutes that pass while
the producer is stopped are never collected on their own. The producer heals the
last `BACKFILL_MINUTES` (default 180) automatically on startup.

For a longer outage, raise it for one run — the window is in **minutes**, and
requests are paginated over Binance's 1000-candle cap:

```bash
BACKFILL_MINUTES=6800 python producer.py
```

Work out the number you need with:

```sql
SELECT ceil(EXTRACT(EPOCH FROM (now() - max(open_time))) / 60) + 100 AS backfill_minutes
FROM crypto.market_candles_1m WHERE symbol = 'ETHUSDT';
```

Leave `consumer.py` running while it catches up, or the candles just queue in
Kafka. Over-shooting the window is harmless — the upsert is idempotent.
...
- **Data gaps** still open whenever the producer isn't running, but they are no
  longer permanent — restarting heals the last `BACKFILL_MINUTES`, and a longer
  outage is recoverable with a one-off larger window (see
  [Recovering after downtime](#recovering-after-downtime)). Grafana draws any
  remaining hole as a break in the line rather than a straight line across it.
